...
Q1_BODY = """
数据诊断分析是量化交易研究的基础环节，旨在在使用数据进行技术指标计算和策略分析之前，全面了解数据的质量状况和统计特征。本次分析以比亚迪(002594.SZ)过去一年的日线交易数据为对象，共包含{n_records}个交易日的观测值（数据范围：{date_start}至{date_end}），数据字段包括交易日期(trade_date)、开盘价(open)、最高价(high)、最低价(low)、收盘价(close)、前收盘价(pre_close)、涨跌额(change)、涨跌幅(pct_chg)、成交量(vol)和成交额(amount)。
...
其次计算描述性统计量。利用df.describe()方法对开盘价、最高价、最低价、收盘价和成交量五个核心字段进行描述性统计分析，统计量包括计数(count)、均值(mean)、标准差(std)、最小值(min)、第一四分位数(25%)、中位数(50%)、第三四分位数(75%)和最大值(max)。从统计结果可以看出，收盘价的均值为{close_mean:.2f}元，标准差为{close_std:.2f}元，反映了该股票在观察期内价格波动幅度较大。最小值{close_min:.2f}元与最大值{close_max:.2f}元之间的巨大差异，主要源于2025年7月29日实施的高送转除权除息（1:3.0），除权前价格在300元以上，除权后降至110元左右。成交量的均值为{vol_mean:,.0f}手，标准差为{vol_std:,.0f}手，表明交易活跃度也存在较大波动。

综上所述，该数据集完整性良好，无缺失值，描述性统计量真实反映了比亚迪股票在过去一年中的价格和成交量分布特征。需要注意的是，除权除息导致的价格跳变属于正常市场事件，在后续技术指标分析中需要特别关注其对指标计算的影响。
""".strip()
